...
bool robot_move(const ROBOT_MOVEMENT move_type)
{
    if (move_type == STOP) {
        ...
        motor_command.angular.z = 0.0;
        motor_command.linear.x = 0.2; //was .5
    }

    else if (move_type == BACKWARD) {
        ROS_INFO("[ROBOT] I'm going back! \n");
        motor_command.linear.x = -0.2; //was -.75
        motor_command.angular.z = 0.0;
    }
...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "wall_follower"); //changed from node to wall_follower
    ...
    motor_command_publisher = n.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/navi", 100); //changed from /robotactuate to /cmd_vel

    // Subscribe to the /scan topic and call the laser_callback function
    laser_subscriber = n.subscribe("/scan", 1000, laser_callback); //changed from laserscan to scan
